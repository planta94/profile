...
## 5. Responsive Experience Timeline Layout

The timeline in the career experience section is fully responsive. To avoid breaking layout alignments or causing word-wrapping issues on mobile viewports (e.g., widths under `640px`), ensure that styling modifications respect the following specifications:

- **Timeline Alignment**: On mobile, the vertical line `.timeline-line` is aligned at `left: 1rem`, the indicator `.timeline-dot-wrapper` is positioned at `left: 0.45rem` (centered on the line) and `top: 1.6rem`, and the `.timeline-item` padding-left is reduced to `2rem`.
- **Card Header Layout**: The `.card-header` changes from a flex-row to a vertical column (`flex-direction: column; align-items: flex-start; gap: 0.5rem; padding-right: 2.2rem;`) to give the role title and company name full width and prevent tight text wrapping.
- **Expand Icon Positioning**: The expand icon (`.expand-icon`) must be absolutely positioned on mobile (`position: absolute; right: 0; top: 0.2rem;`) relative to the `.card-header` container.
